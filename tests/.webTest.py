...
def print_test(msg, status="INFO"):
    color = Fore.BLUE if status == "INFO" else Fore.GREEN if status == "PASS" else Fore.RED
...
def test_connection(path="/", method="GET", body=None, headers=None, expect_status=None, 
    ...
    try:
        ...
        if expect_status and status != expect_status:
            ...
            raise Exception (f"{method} {path} failed: {e}", "FAIL")
...
def test_basic_http_methods():
    print("=== Testing Basic HTTP Methods ===")
...
def test_large_request_body():
    try:
        print("=== Testing Large Request Bodies ===")
        ...
        print("Testing various request sizes to find server limit...")
    ...
    if uploaded_files:
        print(f"Cleaning up {len(uploaded_files)} uploaded test files...")
    ...
    if failed_size > 0:
        print("Testing server stability after large request rejection...")
...
def test_request_size_edge_cases():
    print("=== Testing Request Size Edge Cases ===")
    ...
    if uploaded_files:
        print(f"Cleaning up {len(uploaded_files)} edge case test files...")
...
def test_reasonable_upload_limits():
    print("=== Testing Reasonable Upload Handling ===")
...
def test_keep_alive():
    print("=== Testing Keep-Alive Connections ===")
...
def test_cgi_functionality():#TODO: check PASS
    try:
        print("=== Testing CGI Functionality ===")
...
def test_redirection():
    try:
        print("=== Testing HTTP Redirections ===")
...
def test_server_resilience():
    ...
    print("=== Testing Server Resilience ===")
...
def test_different_ports():
    try:
        print("=== Testing Multiple Ports (if configured) ===")
...
if __name__ == "__main__":
    try:
        ...
        tests = [
            ("Test basic HTTP methods", test_basic_http_methods()),
            ("Test Error handling", test_error_handling()),
            ("Test MIME types", test_mime_types()),
            ("Test HTTP headers", test_http_headers()),
            ("Test multipart Upload", test_multipart_upload()),
            ("Test dir listing", test_directory_listing()),
            ("Test redir", test_redirection()),
            ("Test CGI", test_cgi_functionality()),
            ("Test path traversal security", test_path_traversal_security()),
            ("Test keep-alive", test_keep_alive()),
            ("Test chunked", test_chunked_transfer()),
            ("Test large request body", test_large_request_body()),
            ("Test request size edeg cases", test_request_size_edge_cases()),
            ("Test upload limits", test_reasonable_upload_limits()),
            ("Test different ports", test_different_ports()),
            ("Test concurrent connections", test_concurrent_connections()),
            ("Test server resilience", test_server_resilience()),
            ("Test stress test", test_stress_test())
        ]
